...
class IconSetCommand extends GeneratorCommand
{
    ...
    protected function execute($input, $output): int
    {
        ...
        $question = new ChoiceQuestion('What type of source this set is going to use?', [
            'font',
            'dir',
            'upload',
            'json',
        ...
        if ($setType == 'font' || $setType == 'json') {
            ...
            $fs->copy(
                BASE_PATH.'/vendor/goldfinch/icon-field/components/'.$schemaTemplate,
        ...
        if ($setType == 'dir') {
            ...
            if (! $fs->exists($path)) {
                $createSource = $this->askStringQuestion(
                    'The folder `'.$path.'` does not exist. Would you like to create it? [y/n]',
                    $input,
                    $output,
                    'y'
                );
            ...
            if (! $fs->exists($path)) {
                ...
                $io->info(
                    'Youn need to create `'.
                        ...
                        '` dir in `'.
                        ASSETS_DIR.
                        '` through CMS (/admin/assets). Make sure your uploaded icons in this folder are published.'
                );
            }
        }
